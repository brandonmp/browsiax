...
// https://github.com/brave/browser-laptop/blob/f44924472cf210b0b3efae352d4af53e74c5da5c/js/lib/urlutil.js
'use strict';
...
const UrlUtil = {
   ...
   */
    getViewSourceUrlFromUrl: function(input) {
        if (
            (!UrlUtil.isHttpOrHttps(input) && !UrlUtil.isFileScheme(input)) ||
            UrlUtil.isImageAddress(input)
        ) {
            return null;
        }
        if (UrlUtil.isViewSourceUrl(input)) {
   ...
   */
    getHostnamePatterns: function(url) {
        ...
        for (var i = 2; i <= segmented.length - 2; ++i) {
            hostPatterns.push(
                '*.' + segmented.slice(i, segmented.length).join('.')
            );
        }
        return hostPatterns;
    },
   ...
   */
    ...
    isLocalFile: function(origin) {
        ...
        return (
            origin &&
            localFileOrigins.some(localFileOrigin =>
                origin.startsWith(localFileOrigin)
            )
        );
    },
